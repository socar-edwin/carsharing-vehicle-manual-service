"""
통합 챗봇 - 차량 매뉴얼 + Help Center

차량 조작 질문: 차종 매뉴얼에서 검색
일반 서비스 문의: Help Center에서 검색
"""
...
@st.cache_resource
def load_chatbot():
    """통합 챗봇 인스턴스 로드 (차량 매뉴얼 + Help Center)"""
    ...
    from src.help_center_retriever import HelpCenterRetriever

    VEHICLE_VECTORSTORE_DIR = PROJECT_ROOT / "vectorstore" / "vehicle_manuals_chroma"
    HELP_CENTER_VECTORSTORE_DIR = PROJECT_ROOT / "vectorstore" / "help_center_chroma"

    try:
        # 1. 차종 매뉴얼 인덱스 로드
        vehicle_index = load_vehicle_index(persist_dir=VEHICLE_VECTORSTORE_DIR)
        vehicle_retriever = VehicleRetriever(index=vehicle_index, default_top_k=5)
        logger.info("Vehicle manual index loaded")

        # 2. Help Center 인덱스 로드 (경량화 버전 - LlamaIndex 미사용)
        help_center_retriever = HelpCenterRetriever(persist_dir=HELP_CENTER_VECTORSTORE_DIR, default_top_k=5)
        logger.info("Help Center index loaded")

        # 3. 통합 Chatbot 생성
        chatbot = VehicleChatbot(
            retriever=vehicle_retriever,
            help_center_retriever=help_center_retriever,  # Help Center 활성화
...
def main():
    ...
    if not is_from_url:
        with st.sidebar:
            ...
            display_car_name = selected_vehicle
            ...
            **📚 통합 모드 활성화**
            ...
            이 챗봇은 다음 질문에 모두 답변합니다:
            - 🚗 **차량 조작**: 시동, 충전, 블루투스 등
            - 📋 **서비스 문의**: 예약, 취소, 사고, 보험 등
            """)
    ...
    st.markdown('<div class="main-header">쏘카 통합 챗봇</div>', unsafe_allow_html=True)
    if display_car_name:
        st.markdown(f"**{display_car_name}** 조작 방법 및 **쏘카 서비스** 문의에 답변합니다!")
    else:
        st.markdown("**차량 조작 방법** 및 **쏘카 서비스** 문의에 답변합니다!")
    ...
    if not is_loaded:
        ...
        st.code("python -m src.vehicle_indexing && python -m src.help_center_indexing", language="bash")
        return
        ...
        with st.chat_message(message["role"], avatar=avatar):
            ...
            if message.get("sources") and message["sources"][0].get('url'):
                with st.expander("📖 출처 보기"):
                    # 차량 매뉴얼 출처
                    st.markdown(f'<span class="vehicle-badge">🚗 차량 매뉴얼</span>', unsafe_allow_html=True)
                    st.markdown(f"🔗 [{display_car_name} 이용 안내]({message['sources'][0]['url']})")
            # Help Center 출처
            if message.get("help_center_sources"):
                with st.expander("📋 서비스 도움말 보기"):
                    st.markdown(f'<span class="help-center-badge">📋 Help Center</span>', unsafe_allow_html=True)
                    for hc_src in message["help_center_sources"]:
                        st.markdown(f"🔗 [{hc_src.get('title', '도움말')}]({hc_src.get('url', '')})")
    ...
    if show_suggestions:
        ...
        all_questions = vehicle_questions + service_questions
        ...
        with st.chat_message("assistant", avatar=ASSISTANT_AVATAR):
            with st.spinner("답변을 생성하고 있습니다..."):
                try:
                    ...
                    help_center_sources = result.get("help_center_sources", [])
                    is_general = result.get("is_general_inquiry", False)
                    ...
                    # Help Center 출처 표시
                    if help_center_sources:
                        with st.expander("📋 서비스 도움말 보기"):
                            st.markdown(f'<span class="help-center-badge">📋 Help Center</span>', unsafe_allow_html=True)
                            for hc_src in help_center_sources:
                                st.markdown(f"🔗 [{hc_src.get('title', '도움말')}]({hc_src.get('url', '')})")
